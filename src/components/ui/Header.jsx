...
import { useDashboardActions, useDashboardUIState } from '../../context/DashboardContext';

function Header() {
  const { darkMode, role } = useDashboardUIState();
  const { setDarkMode, setRole } = useDashboardActions();

  return (
    <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/92 backdrop-blur-xl dark:border-slate-700 dark:bg-slate-900/82">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3.5 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-extrabold tracking-tight text-slate-900 dark:text-slate-100 sm:text-2xl">FinanceTrack</h1>
            <p className="text-xs font-medium uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">Personal Finance Command Center</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <label className="field-control hidden items-center gap-2 rounded-lg px-3 py-2 text-sm sm:flex">
      ...
      <div className="border-t border-slate-200/70 px-4 py-3 dark:border-slate-700 sm:hidden">
        <label className="field-control flex items-center gap-2 rounded-lg px-3 py-2 text-sm">
          <Shield className="h-4 w-4 text-slate-500" />
